...
- Offline unit tests: `lua test/unit/unit.lua` — exercises deterministic configuration defaults/validation/migration/reset, event dispatch, frame discovery, target-of-target combat deferral and the small legacy Mouse-overlay removal regression check. It reuses `test/perf/harness.lua`; no second WoW mock is maintained.
