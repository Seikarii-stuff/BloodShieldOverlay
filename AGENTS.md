...
- Offline smoke test: `lua test/perf/smoke.lua` — validates loading, current public APIs, event coalescing, menu initialization, combat-deferred refresh, bounded frame discovery, and module loading. The assertion count is intentionally not hard-coded in this guide; run the test to obtain the current result.
...
- Synthetic benchmark: `lua test/benchmark/benchmark.lua 100000` — measures dispatch, overlay updates, discovery throughput, player/group resource paths, listener count and Lua heap delta.
